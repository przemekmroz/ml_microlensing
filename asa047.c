...
*/
  for ( ; ; )
  {
    ...
    y[n] = fn ( start, xdata, ydata, ydatae, outlier, npdata );
    *icount = *icount + 1;
    ...
    for ( j = 0; j < n; j++ )
    {
      ...
      y[j] = fn ( start, xdata, ydata, ydatae, outlier, npdata );
      *icount = *icount + 1;
      start[j] = x;
    }
/*                 
...
*/
      ...
      ystar = fn ( pstar, xdata, ydata, ydatae, outlier, npdata );
      *icount = *icount + 1;
/*
...
*/
      if ( ystar < ylo )
      {
        ...
        y2star = fn ( p2star, xdata, ydata, ydatae, outlier, npdata );
        *icount = *icount + 1;
/*
  Check extension.
*/
...
*/
          if ( y[ihi] < y2star )
          {
            for ( j = 0; j < nn; j++ )
            {
              ...
              y[j] = fn ( xmin, xdata, ydata, ydatae, outlier, npdata );
              *icount = *icount + 1;
            }
            ylo = y[0];
            ilo = 0;
...
*/
    ...
    *ifault = 0;
    ...
    for ( i = 0; i < n; i++ )
    {
      del = step[i] * eps;
      xmin[i] = xmin[i] + del;
      z = fn ( xmin, xdata, ydata, ydatae, outlier, npdata );
      *icount = *icount + 1;
      if ( z < *ynewlo )
      {
        *ifault = 2;
